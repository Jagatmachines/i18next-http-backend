...
import { readFile, removeFile } from './readFile.js'
...
class Backend {
  ...
  read (language, namespace, callback) {
    this._readNamespaceFile(language, namespace, (err, res) => {
      if (res) {
        this._readAny([language], language, [namespace], namespace, callback, res)
      }
      if (err) callback(new Error('No Main Translation file available!'), false)
    })
  }

  _readNamespaceFile (language, namespace, callback) {
    let localLoadPath = this.options.localLoadPath
    if (typeof this.options.localLoadPath === 'function') {
      localLoadPath = this.options.localLoadPath(language, namespace)
    }
    const filename = this.services.interpolator.interpolate(localLoadPath, { lng: language, ns: namespace })
  ...
  _readAny (languages, loadUrlLanguages, namespaces, loadUrlNamespaces, callback, payload) {
    ...
    loadPath.then(resolvedLoadPath => {
      ...
      this.loadUrl(url, callback, loadUrlLanguages, loadUrlNamespaces, payload)
    })
  }
